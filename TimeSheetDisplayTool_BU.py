...
def UILayout():
    ...
    for i in range(0, 96):
        ...
        sColourWE = 'TimeSheet_Chart_OffWork_BG'
        if 36 < i <92: 
        ...
        for h in range(0, 98, 4):
            if i == h:
                ...
                sColourWE = 'TimeSheet_Chart_OffWork_Line'

        for h in range(36, 92, 4):
            if i == h:
        ...
        if i == 48:
            ...
            sColourWE = 'TimeSheet_Chart_Noon_Line'
        # Get time
        iMin = i * 15
        iH = iMin / 60
        iM = iMin % 60
        ...
        cmds.separator( height = oUI.iRowHeight, style = 'none', w = oUI.Div[0][0] ),
        cmds.text('uiSun1_%s'%sTime, l = ' ', h = oUI.iRowHeight, w = oUI.Div[0][1]+oUI.Div[0][1], bgc = UIColourControl.keywordColour(sColourWE)),
        ...
        cmds.text('uiSat1_%s'%sTime, l = ' ', h = oUI.iRowHeight, w = oUI.Div[0][19]+oUI.Div[0][1], bgc = UIColourControl.keywordColour(sColourWE)),
    ...
    cmds.text('uiSat2_Total', l = ' ', h = oUI.iRowHeight, w = oUI.Div[0][20], bgc = UIColourControl.keywordColour(sColour)),
    ...
    cmds.textField('uiMon2_TimeSheet', tx = ':00:00', h = oUI.iRowHeight, w = oUI.Div[0][1]+oUI.Div[0][1], bgc = UIColourControl.keywordColour(sColour), ec = partial(DayAdjust,'uiMon2_TimeSheet') ),
...
def DayAdjust(sUITimeSheet, *args):
    ...
    iTotalMin = int(sTotal.split(':')[0]) * 60 + int(sTotal.split(':')[1])
    iTotalTimeSheet = int(sTimeSheet.split(':')[0]) * 60 + int(sTimeSheet.split(':')[1])
    ...
    # Add total work hour from all shots for this day.
    aHour = []
    for w in aWeek:
        for i in range(len(aShotList)):
            iHour = 0
    ...
    # Continue Here. logic to average out extra hours.
    #aHour = [0,0, 45, 60, 90]
    #iTotal = iTotalMin

    #iAdj = iTotal - 180

    #iDifference = (iTotal - iAdj) if iAdj > 0 else 0
    #print aHour, sum(aHour)
    #print iTotalMin
    #print iDifference
    ...
    if iDifference:
        ...
        while iStop == 0:
            ...
            iAdd = 15 if iDifference < 0 else -15

            for i, h in enumerate(aHour[:]):
                if h == 0:
                    pass
                elif iAdd < 0 and h < 60:
                    if sum(aHour) <= (len(aHour) - aHour.count(0))* 60:
                        aHour[i] = aHour[i] + iAdd
                        iDifference += iAdd
                        if iDifference == 0:
                            break
                else:
        ...
        cmds.textField('%s_Temp'%(sWeek), e = True, tx = '%s:%s'%(str(sum(aHour)/60).zfill(2), str(sum(aHour)%60).zfill(2)))
...
def UIReBuild():
    ...
    aUICOLOUR = []
    for i in range(10):
        aUICOLOUR.append('TimeSheet_Seq1_Shot%s'%(i+1))
    ...
    for w, a in enumerate(aWeekInfo):
        ...
        for b in a[1]:
            #print '     ',b[0]
            sColour = aUICOLOUR[aShotList.index(b[0])]
            print sColour 
            ...
            for c in b[1]:
                ...
                while iTimeCheck:
                    if ':'.join(aStart) == c[1]:
                        iTimeCheck = 0
                    cmds.text('%s1_%s'%(aUIWEEK[w], ':'.join(aStart)), e = True, l = ' ', bgc = UIColourControl.keywordColour(sColour)),
                    if aStart[1] == '45':
                        aStart[1] = '00'
            ...
            cmds.text('uiSun1_shot%s'%(aShotList.index(b[0]) ) , e = True, bgc = UIColourControl.keywordColour(aUICOLOUR[aShotList.index(b[0])]))
            cmds.textField('%s2_shot%s'%(aUIWEEK[w], aShotList.index(b[0]) ) , e = True, tx = str(atShotHours[-1])[:-3], bgc = UIColourControl.keywordColour(aUICOLOUR[aShotList.index(b[0])]))
