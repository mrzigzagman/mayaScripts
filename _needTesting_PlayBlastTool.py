...
class UIBuilder:
    ...
    self.sScriptPath = '/vol/transfer/dyabu/Scripts/mayaScripts/%s.py'%self.sScriptName
    self.StudioSettings = imp.load_source(self.sScriptName, self.sScriptPath)

    # Getting info using StudioSettings
    self.aShotInfo = self.StudioSettings.ShotInfo(1,1) # (1,1) = (Folder Creation, Print paths.)
    self.sRvPath = self.aShotInfo[0] + 'Active.rv'
    ...
    iTarget =  self.dDict['ConvertToolT'][1:].index(1)+1
    ...
    shutil.copy(sSourcePath+'/' + tf, sTargetPath+'/'+'.'.joing(aName))
    ...
    def UIConvertTool_DisplayChecker(self):
        ...
        if self.dDict['ConvertTool]:
            for i in range(5):
                ...
                iVal = self.dDict['CovertToolTo'][i]
                ...
                iVal = self.dDict['ConverToolFrom'][i]
    ...
    def UIBGColor(self, sColour = 'red'):
        dColour = { 'tone1':(0.822, 0.967, 1.000),
    ...
    def UILayout(self):
        ...
        ##Row
        self.UIDivision([1, 1.7, .6, .3, 1.3, 1.3, .6, .4, .3, 1.2], None, 0)
        aRow = [
        ...
        cmds.button(label = 'Range', h = self.iRowHeight, w = self.Div[0][10], bgc = self.UIBGColour('RangeButton'), command = partial(self.UIButton_RVInfoToMaya, 'oRangeButton')),
        ...
        cmds.button(label = 'Current', h = self.iRowHeight, w = self.Div[0][9], bgc = self.UIBGColour('CurrentButton'), command = partial(self.UIButton_Open, 'Current')),
        ]
        self.UIAddRow(aRow)
        ...
        cmds.button('oUIConvert', label = 'Convert', h = self.iRowHeight, w = self.Div[0][1], bgc = self.UIBGColour('ConvertToolButtonOff'), enableBackground = False, command = partial(self.UIButton_ExtraToolExpand, 'Convert')),
        ...
        cmds.button(label = 'Marked', h = self.iRowHeight, w = self.Div[0][7], bgc = self.UIBGColour('MarkedButton'), command = partial(self.UIButton_ExtraToolExpand, 'Marked')),
        ]; self.UIAddrow(aRow)
        ...
        if self.dDict['ConvertTool']:
            ...
            separator( height = self.iRowHeight, style = 'none', w = self.Div[0][3]),
            cmds.button('oUIConvert5', label = '2', h = self.iRowHeight, w = self.Div[0][4], bgc = self.UIBGColour('darkgray'), command = partial(self.UIConvertTool, '5')),
            ...
            cmds.separator(height = self.iRowHeight, style = 'none', w = self.Div[0][3]),
            cmds.button('oUIConvert7', label = '7', h = self.iRowHeight, w = self.Div[0][4], bgc = self.UIBGColour('darkgray'), command = partial(self.UIConvertTool, '7')),
            cmds.button('oUIConvert8', label = '8', h = self.iRowHeight, w = self.Div[0][5], bgc = self.UIBGColour('darkgray'), command = partial(self.UIConvertTool, '8')),
        ...
        if self.dDict['CalcTool'];
            ...
            cmds.textField(tx = '       - Range Checker - ', h = self.iRowHeight, w = self.Div[0][5], ed = Falsee), #bgc = self.UIBGColour('tone2')),
    ...
    def UICalcTool(self, *args):
        aVars = ['oCalcTarget', 'oCalMoveTo', 'oCalcFirstFrame', 'oCalcLastFrame']
        aAdjVars = []
    ...
    def UIButton_RVInfoTomay(self, sType, *args):
    ...
    def UIButton_InOut(self, sInOut, *args):
        ...
        iCurrent = int(cmds.currentTime(q = Time))
        if sInOut == 'Start':
...
# __________________________COntinue here:
